...
use \Zend_Form_Decorator_Abstract,
    \Wednesday\Form\Element\Groups\StandardFooter;
 ...
 */
class Wednesday_Form_Decorator_FormTabs extends Zend_Form_Decorator_Abstract
{
    /**
    ...
    protected $_defaults = array(
        ...
        'markupListItemStart'     => '<li id="" class="">',
        'markupListStart'         => '<ul class="frmtabs nav nav-tabs">',
    );
...
//        return $markup . $this->getSeparator() . $content;
     ...
     * @return string
     */
    public function getMarkupListItemStart($id = '', $class = '')
    {
        ...
        $markupLiItemStart = $this->_markupListItemStart;
        if($id != '') {
            $markupLiItemStart = str_replace('id="', 'id="tab-'.$id.'', $markupLiItemStart);
        }
        if($class != '') {
            $markupLiItemStart = str_replace('class="', 'class="'.$class.' ', $markupLiItemStart);
        }
        return $markupLiItemStart;
    }
    ...
    protected function _recurseForm(Zend_Form $form, Zend_View_Interface $view)
    {
        $content = '';
        $i = 0;
        $max = count($form->getSubForms())-2;
        foreach ($form->getSubForms() as $subitem) {
            if($subitem->getName() != StandardFooter::NAME) {
                if ($subitem instanceof Zend_Form) {
                    $errorBadge = $this->getErrorTags($subitem);
                    $class = ($i==0)?'ui-state-active':'';
                    if($subitem->count() == 0) {
                        $class .= " ui-state-disabled";
                    }
                    $content .= $this->getMarkupListItemStart($subitem->getName(),$class)
                             . '<a href="#'.$subitem->getName().'">'.$subitem->getLegend().' '.$errorBadge.'</a>'
                             .  $this->getMarkupListItemEnd();
                    $i++;
                }
            }
    ...
    protected function getErrorTags($subform, $ascount = false) {
//        echo $subform->getName();
//        var_dump($subform->getErrors());
//        var_dump($subform);
        $count = 0;
        if(!isset($subform)) {
            return $count;
        }
        foreach($subform->getErrors() as $key => $errors) {
            if(empty($errors)) {
                continue;
            } else {
                $elem = $subform->getElement($key);
                if($elem) {
                    $count++;
                } else {
                    $subsubform = $subform->getSubform($key);
//                    echo $key;
//                    var_dump($subsubform);
                    $count += $this->getErrorTags($subsubform,true);
                }
            }
        }
        if($ascount==true) {
        ...
        return ($count == 0)?"":'<span class="badge badge-important">'.$count.'</span>';
    }
